...
const createInstagramRequest = (cookieHeader) => async (pathname, params = {}) => {
...
const throwForResponse = async (response, action) => {
  ...
  if ([401, 403, 429].includes(response.status)) {
    throw new Error(`Instagram rechazó ${action} (HTTP ${response.status}).`)
  }
  throw new Error(`Error ${action}: HTTP ${response.status}`)
}
